...
#momo-status-bar .msb-tab:hover { color: #9a9ab0; background: rgba(255,255,255,0.03); }
...
#momo-status-bar .msb-panel {
  display: none;
  background: linear-gradient(180deg, #16162a 0%, #121220 100%);
  padding: 14px 16px;
}
...
#momo-status-bar .msb-secask-option:hover {
...
#momo-status-bar .msb-closet-page { font-size: 11px; color: #787890; font-weight: 400; }
#momo-status-bar .msb-closet-pages { display: flex; gap: 8px; justify-content: center; padding: 8px 0 4px; }
#momo-status-bar .msb-closet-page-btn { background: rgba(255,255,255,0.06); border: 1px solid rgba(232,160,191,0.12); border-radius: 8px; color: #b0b0c8; font-size: 13px; padding: 4px 14px; cursor: pointer; font-family: inherit; transition: all 0.15s; }
#momo-status-bar .msb-closet-page-btn:hover { background: rgba(255,107,157,0.12); border-color: rgba(255,107,157,0.3); color: #ff6b9d; }
...
#momo-status-bar .msb-tag-color { background: rgba(162,227,196,0.12); color: #7ec8a0; }
#momo-status-bar .msb-tag-trait { background: rgba(255,107,107,0.15); color: #ff6b6b; font-weight: 700; }
#momo-status-bar .msb-tag-erotic { background: rgba(255,107,157,0.12); color: #ff6b9d; font-weight: 700; }
#momo-status-bar .msb-tag-material { background: rgba(162,140,220,0.12); color: #b8a0e0; }

#momo-status-bar .msb-schedule-panel { padding: 0; min-height: 400px; }
#momo-status-bar .msb-schedule-empty { padding: 24px 14px; color: #787890; font-size: 12px; text-align: center; }
#momo-status-bar .msb-sched-date-head { padding: 12px 14px 6px; font-size: 13px; font-weight: 600; color: #e0dee0; }
#momo-status-bar .msb-sched-card-box { margin: 0 12px 6px; border: 1px solid rgba(232,160,191,0.10); border-radius: 10px; background: rgba(255,107,157,0.03); overflow: hidden; }
#momo-status-bar .msb-sched-done { opacity: 0.35; }
#momo-status-bar .msb-sched-done .msb-sched-cell-title { text-decoration: line-through; }
#momo-status-bar .msb-sched-divider { height: 1px; background: rgba(232,160,191,0.08); margin: 0; }
#momo-status-bar .msb-sched-row { display: grid; grid-template-columns: 100px 1fr 1fr; gap: 0 8px; padding: 8px 14px 8px 18px; font-size: 13px; align-items: baseline; }
#momo-status-bar .msb-sched-cell-time { font-size: 12px; color: #787890; font-weight: 500; white-space: nowrap; }
#momo-status-bar .msb-sched-cell-title { color: #e8e6e3; }
#momo-status-bar .msb-sched-cell-loc { font-size: 12px; color: #6a6a80; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
const DEFAULTS = {
  ...
  '日程': { '日期': '2024-09-01', '星期': '周日', '当前任务': { '任务名': '无', '类型': '—', '阶级': '—', '目标': '—', '额外要求': '—', '报酬': 0 }, '用户日程': [
    { '时间': '', '事项': '上半学期开学报到', '地点': '海平大学行政楼', '类型': '固定', '完成': false, '日期': '2024年9月1日' },
  ] },
  '社交': {
    '联系人': {
  ...
  '衣柜': {
    '当前穿着': ['居家蕾丝睡裙'],
    '可选服装': {
      '居家蕾丝睡裙': { '来源': '衣柜', '部位标签': ['外套'], '风格标签': ['居家'], '材质标签': ['蕾丝'], '颜色标签': ['白色'], '特质标签': ['薄透'], '色情值': 60, '价格': 0, '说明': '过膝长款睡裙，收腰X版型，贴合墨墨腰身曲线，凸显胸部与臀部的落差；深V领口开到胸口中段，锁骨与乳沟清晰露出，蕾丝花边领沿锁骨展开；短袖泡泡袖，袖口微收带细碎荷叶边。白色蕾丝面料通体半透，强光下乳晕及腰腹轮廓隐约可见，裙摆到大腿中段，A字散开带细碎蕾丝花边，下摆轻晃时腿根若隐若现。' },
      '休闲纯棉T恤': { '来源': '衣柜', '部位标签': ['内衬'], '风格标签': ['日常', '休闲'], '材质标签': ['纯棉'], '颜色标签': ['白色'], '特质标签': ['宽松'], '色情值': 25, '价格': 60, '说明': '圆领短袖T恤，整体微宽松直筒版型，不贴身不显胸腰曲线，领口圆弧大小刚好露出锁骨根部和一点颈窝；正肩剪裁，短袖袖口刚好包住肩头。纯棉面料，厚度适中不透光，纯白色无花纹。整体感宽松日常，垂感自然。' },
      '休闲运动短裙': { '来源': '衣柜', '部位标签': ['下装'], '风格标签': ['日常', '休闲', '运动'], '材质标签': ['涤纶'], '颜色标签': ['黑色'], '特质标签': ['安全裤'], '色情值': 50, '价格': 120, '说明': '高腰A字短裙，裙长到大腿上段约三分之一处，坐下时裙边会上卷至腿根附近露出大半截大腿；宽腰头设计，腰线在肚脐上方，收拢腰腹。面料为哑光涤纶，纯黑色无花纹、有内衬安全裤同色，裙摆挺括不贴身。整体风格休闲运动，走动能带起裙摆晃动。' },
      '休闲帆布鞋': { '来源': '衣柜', '部位标签': ['鞋'], '风格标签': ['日常', '休闲'], '材质标签': ['帆布'], '颜色标签': ['白色'], '特质标签': ['百搭'], '色情值': 5, '价格': 80, '说明': '经典款白色帆布鞋，低帮系带款，鞋面纯白帆布，鞋底微黄橡胶，鞋侧有两个透气孔。整体轻微做旧，鞋底边缘有日常磨损痕迹。日常百搭，几乎适配所有休闲着装。' },
      '休闲卫裤': { '来源': '衣柜', '部位标签': ['下装'], '风格标签': ['日常', '休闲', '运动'], '材质标签': ['纯棉'], '颜色标签': ['灰色'], '特质标签': ['宽松', '收脚'], '色情值': 15, '价格': 100, '说明': '宽松收脚卫裤，整体直筒垂坠版型，完全遮盖墨墨的腿臀曲线；中高腰松紧抽绳腰头，裤长到脚踝上方，裤脚收口带弹力罗纹。纯棉抓绒面料，厚度适中，纯灰色无花纹，大腿侧有两个斜插口袋。整体宽松舒适，不显身形。' },
      '休闲连帽卫衣': { '来源': '衣柜', '部位标签': ['外套'], '风格标签': ['日常', '休闲', '运动'], '材质标签': ['纯棉抓绒'], '颜色标签': ['灰色'], '特质标签': ['宽大'], '色情值': 10, '价格': 150, '说明': '大号连帽卫衣，整体超宽松落肩版型，几乎完全遮盖墨墨上半身曲线，长度过臀；连帽抽绳设计，领口圆领无拉链，袋鼠口袋位于腹前。纯棉抓绒面料，内里起绒，纯灰色无花纹。袖长盖过手腕半个手掌。整体宽大遮身，不显身材。' },
      '日常紧身牛仔裤': { '来源': '衣柜', '部位标签': ['下装'], '风格标签': ['日常', '休闲'], '材质标签': ['牛仔'], '颜色标签': ['蓝色'], '特质标签': ['弹力', '修身'], '色情值': 40, '价格': 200, '说明': '中腰紧身牛仔裤，整体贴身弹力版型，完美贴合墨墨从腰到脚踝的曲线；中腰设计腰线在肚脐下方，拉链门襟加金属纽扣。弹力牛仔面料，水洗蓝色微做旧无破洞，大腿面和臀部因弹力面料紧贴而勾勒出饱满轮廓，裤脚无开叉刚好盖住脚面。整体风格简约日常。' },
      '日常棉质内衣': { '来源': '衣柜', '部位标签': ['内衣'], '风格标签': ['日常'], '材质标签': ['棉质混纺'], '颜色标签': ['肉色'], '特质标签': ['无痕', '薄杯'], '色情值': 35, '价格': 80, '说明': '肉色无痕内衣套装，包括文胸和三角内裤。文胸为薄杯无钢圈设计，杯面平滑无痕无蕾丝，背扣三排两扣；内裤为低腰三角款，腰头无标签无痕设计。棉质混纺面料，含弹性纤维，贴近肤色，穿在浅色外衣下不会透出明显轮廓。' },
      '日常纯棉内衣': { '来源': '衣柜', '部位标签': ['内衣'], '风格标签': ['日常'], '材质标签': ['棉质'], '颜色标签': ['白色'], '特质标签': ['薄垫'], '色情值': 30, '价格': 70, '说明': '纯白棉质内衣套装，包括文胸和内裤。文胸为薄垫加钢圈设计，杯型挺括有型，聚拢效果明显但不夸张，背扣三排两扣；内裤为平角款，腰头纯棉弹性罗纹。面料为纯棉针织，通体纯白色，穿着浅色外衣时会透出内衣轮廓。' },
      '休闲棉袜': { '来源': '衣柜', '部位标签': ['腿部'], '风格标签': ['日常', '休闲'], '材质标签': ['棉质'], '颜色标签': ['白色'], '特质标签': ['中筒'], '色情值': 30, '价格': 35, '说明': '白色中筒棉袜，袜口到膝盖下方约两指宽处，套口带弹力罗纹不会滑落。纯白棉质面料无明显花纹，脚尖和脚跟处加厚。日常款，穿短裙时袜口会露出一截白色袜边。' },
    },
  },
};
...
let currentAvailableTasks = null;
let closetPage = 0; // 服装模块分页，一页6件
...
function getData() {
  ...
  try {
    ...
    const mvu = st?.Mvu;
    if (mvu) {
      const messageId = st?.getCurrentMessageId?.() ?? -1;
  ...
  try {
    ...
    const stat_data = v?.stat_data || v?.display_data;
    if (stat_data && typeof stat_data === 'object' && Object.keys(stat_data).length > 0) {
      const r = { ...stat_data };
      for (const k of Object.keys(DEFAULTS)) {
        if (!(k in r) || r[k] === undefined || r[k] === null) {
...
function tagClass(slot) {
  ...
  if (['日常', '休闲', '居家', '睡眠', '运动', '通勤', '工作', '情趣'].includes(slot)) return 'msb-tag-style';
  if (['白色', '黑色', '灰色', '肉色', '蓝色', '裸色'].includes(slot)) return 'msb-tag-color';
  if (['蕾丝', '纯棉', '棉质混纺', '涤纶', '帆布', '牛仔', '纯棉抓绒'].includes(slot)) return 'msb-tag-material';
  if (['薄透', '宽松', '安全裤', '百搭', '收脚', '宽大', '弹力', '修身', '无痕', '薄杯', '薄垫', '中筒'].includes(slot)) return 'msb-tag-trait';
  return 'msb-tag-part';
}
...
function renderSchedulePanel(data) {
  const date = data?.['日程']?.['日期'] || '';
  const weekday = data?.['日程']?.['星期'] || '';
  let fixed = data?.['日程']?.['固定日程'] || [];
  let userSched = data?.['日程']?.['用户日程'] || [];
  // stat_data 无数据时使用 DEFAULTS 后备
  if (userSched.length === 0 && fixed.length === 0) {
    const def = DEFAULTS?.['日程']?.['用户日程'] || [];
    if (def.length > 0) userSched = def;
  }
  const isInSemester = data?.['日程']?.['是否学期'];
  const items = [
    ...fixed.map(s => ({
      time: s['时间'] || '', title: s['事项'] || '',
      location: s['地点'] || '', done: s['完成'],
      type: 'fixed', date: '', desc: '',
    })),
    ...userSched.map(s => ({
      time: s['时间'] || '', title: s['事项'] || '',
      location: s['地点'] || '', done: s['完成'],
      type: s['类型'] === '固定' ? 'fixed' : s['类型'] === '约定' ? 'promise' : 'reminder',
      date: s['日期'] || '', desc: s['内容'] || '',
    })),
  ].filter(item => !item.done); // 完成的任务不显示
  items.sort((a, b) => {
  ...
  const groupKeys = Object.keys(groups).sort((a, b) => {
    ...
    return parseScheduleDate(a) - parseScheduleDate(b);
  });
  return `<div class="msb-schedule-panel">
    ${items.length === 0 ? '<div class="msb-schedule-empty">暂无日程安排</div>' : groupKeys.map(key => {
      const g = groups[key];
      const gdate = key === 'default' ? '' : key;
      return `
    ${gdate ? `<div class="msb-sched-date-head">${esc(gdate)}</div>` : ''}
    ${g.items.map((item, idx) => {
      const doneClass = item.done ? ' msb-sched-done' : '';
      // 计算迫近度：距游戏当前时间越近越亮
      const nowDate = parseScheduleDate(data?.['日程']?.['日期'] || '2024年9月1日');
      const itemDate = parseScheduleDate(item.date || '');
      const diff = itemDate - nowDate;
      let bgColor = 'transparent';
      if (diff <= 0) bgColor = 'rgba(255,77,77,0.20)';        // day0 亮红色
      else if (diff <= 1) bgColor = 'rgba(255,107,157,0.20)';  // day1 粉红色
      else if (diff <= 7) bgColor = 'rgba(255,193,7,0.15)';    // day2-7 黄色
      else if (diff <= 30) bgColor = 'rgba(107,207,130,0.12)'; // day7-30 绿色
      // 超过30天保持透明
      return `<div class="msb-sched-card-box${doneClass}" style="background:${bgColor}">
        ${idx === 0 ? '<div class="msb-sched-divider"></div>' : ''}
        <div class="msb-sched-row">
          <span class="msb-sched-cell-time">${item.time ? esc(item.time) : ''}</span>
          <span class="msb-sched-cell-title">${esc(item.title)}</span>
          <span class="msb-sched-cell-loc">${item.location ? esc(item.location) : ''}</span>
        </div>
      </div>`;
    }).join('')}
    `;
...
function renderAppPanel(data) {
  ...
  const hasTask = task?.['任务名'] && task['任务名'] !== '无';
  // 如果已有当前任务，清空内存中的可用任务列表
  if (hasTask && currentAvailableTasks) {
    currentAvailableTasks = null;
    selectedTaskIndex = -1;
  }
  const availableTasks = currentAvailableTasks || data?.['日程']?.['可用任务'];
  const hasAvailable = !hasTask && Array.isArray(availableTasks) && availableTasks.length > 0;
  ...
  <div class="msb-task-list">
    ${availableTasks.map((t, i) => {
      ...
      const baseReward = Number(t['报酬']) || 0;
      const extraText = t['额外要求'] || '';
      const hasExtra = extraText && extraText !== '无';
      const pctMatch = extraText.match(/[+＋](\d+)%/);
      const extraPct = pctMatch ? Number(pctMatch[1]) : 0;
      const extraAmount = extraPct > 0 ? Math.round(Number(baseReward) * extraPct / 100) : 0;
      return `
    <div class="msb-task-card msb-task-option${i === selectedTaskIndex ? ' msb-task-selected' : ''}" data-task-idx="${i}">
      ...
      ${hasExtra ? `<div class="msb-task-row"><span class="msb-task-label">额外要求</span><span class="msb-task-value">${esc(extraText)}</span></div>` : '<div class="msb-task-row"><span class="msb-task-label">额外要求</span><span class="msb-task-value" style="color:#787890">无</span></div>'}
      <div class="msb-task-row"><span class="msb-task-label">报酬</span><span class="msb-task-value" style="color:#a2e3c4;font-weight:600">${baseReward}元${hasExtra && extraPct > 0 ? `<span style="color:#9a9ab0;font-weight:400;font-size:11px">（完成额外要求+${extraPct}%=${baseReward + extraAmount}元）</span>` : ''}</span></div>
    </div>`;
    }).join('')}
  </div>
  ...
  function renderCard(name, item, isOn) {
    ...
    return `<div class="${cardClass}" data-closet-name="${esc(name)}">
      ...
      <div class="msb-closet-card-detail" style="display:none">
        <div class="msb-closet-extra-tags">${materialTags}${traitTags}</div>
        <div class="msb-closet-detail-desc">${esc(desc)}</div>
      </div>
    </div>`;
  }

  const wearing = sorted.filter(n => {
    const currentWear = data?.['衣柜']?.['当前穿着'];
    if (!Array.isArray(currentWear)) return momoWearList.includes(n);
    return currentWear.includes(n);
  });
  const available = sorted.filter(n => {
    const currentWear = data?.['衣柜']?.['当前穿着'];
    if (!Array.isArray(currentWear)) return !momoWearList.includes(n);
    return !currentWear.includes(n);
  });
  const PER_PAGE = 6;
  const sortedAvailable = available.filter(n => allClothing[n]);
  const totalPages = Math.max(1, Math.ceil(sortedAvailable.length / PER_PAGE));
  if (closetPage >= totalPages) closetPage = 0;
  const pageItems = sortedAvailable.slice(closetPage * PER_PAGE, (closetPage + 1) * PER_PAGE);
  const pageHtml = pageItems.map(n => renderCard(n, allClothing[n], false)).join('');
...
<div class="msb-closet-columns">
  <div class="msb-closet-col">
    ...
    <div class="msb-closet-col">
      <div class="msb-closet-col-title">🧺 衣柜${totalPages > 1 ? ` <span class="msb-closet-page">${closetPage + 1}/${totalPages}</span>` : ''}</div>
      ${pageHtml || '<div class="msb-closet-empty">衣柜为空</div>'}
      ${totalPages > 1 ? `<div class="msb-closet-pages"><button class="msb-closet-page-btn" data-page="prev">◀</button><button class="msb-closet-page-btn" data-page="next">▶</button></div>` : ''}
    </div>
</div>
...
function renderSecaskPanel() {
  try {
    ...
    for (let i = ctx.chat.length - 1; i >= 0; i--) {
      ...
      if (!msg || msg.is_user) continue;
      const text = msg.mes || '';
      const secaskMatch = text.match(/<secask>([\s\S]*?)<\/secask>/);
      if (secaskMatch) {
        const content = secaskMatch[1];
        ...
        const html = options.map((opt, i) =>
          `<div class="msb-task-card msb-secask-option" data-idx="${i}">
            <div class="msb-task-option-header">
              <span class="msb-task-option-name">${esc(opt.desc.substring(0, 90))}</span>
            </div>
          </div>`
        ).join('\n');
        return `<div style="padding:10px 14px;border-top:1px solid rgba(232,160,191,0.12);background:linear-gradient(180deg,#1e1e32 0%,#1a1a2e 100%);border-radius:16px">
          ...
          <div class="msb-task-list">${html}</div>
        </div>`;
      }
    }
  } catch (e) {}
...
<div id="momo-status-bar">
  ...
  <div class="msb-tabs">
    ...
    <div class="msb-tab" data-tab="schedule"><span class="msb-tab-icon">📅</span>日程</div>
  ...
  <div class="msb-panel schedule">${renderSchedulePanel(data)}</div>
...
function injectStatusBar() {
  ...
  if (mesTexts.length > 0) {
    ...
    const secaskFooter = bar.querySelector('.msb-footer');
    if (secaskFooter) secaskFooter.outerHTML = renderSecaskPanel();
  });
}
...
function refreshAllBars() {
  ...
  pDoc.querySelectorAll('#momo-status-bar').forEach(bar => {
    ...
    if (appPanel) {
      appPanel.innerHTML = renderAppPanel(data);
      if (selectedTaskIndex >= 0) {
        const taskCard = appPanel.querySelector(`.msb-task-option[data-task-idx="${selectedTaskIndex}"]`);
        if (taskCard) {
          taskCard.classList.add('msb-task-selected');
          const confirmBtn = appPanel.querySelector('.msb-task-confirm-btn');
    ...
    const schedPanel = bar.querySelector('.msb-panel.schedule');
    if (schedPanel) schedPanel.innerHTML = renderSchedulePanel(data);
...
function start() {
  ...
  (async () => {
    ...
    // 等待消息楼层变量的 stat_data 被正确设置
    try {
      await new Promise((resolve, reject) => {
        let retries = 0;
        const maxRetries = 50; // 50 * 500ms = 25s timeout
        const timer = setInterval(() => {
          ...
          if (sd && typeof sd === 'object' && Object.keys(sd).length > 0) {
            ...
            resolve(); // 超时也继续，可能显示后备数据
          }
        }, 500);
      });
    } catch (e) {
      console.warn('[MomoStatusBar] stat_data wait failed', e);
    }
    injectStatusBar();
    refreshAllBars();
  })();

  // Startup fallback: 每隔 3s 尝试注入，最多 3 次
  ...
  const startupTimer = setInterval(() => {
    ...
    if (++startupRetries >= 3) clearInterval(startupTimer);
  }, 3000);
  ...
  pDoc.addEventListener('click', function (e) {
    ...
    const hasTask = Array.isArray(currentAvailableTasks) && selectedTaskIndex >= 0 && selectedTaskIndex < currentAvailableTasks.length;

    if (!hasClothing && !hasContact && !hasTask && !isContactBtn) return;
    ...
    if (hasTask && Array.isArray(currentAvailableTasks) && selectedTaskIndex >= 0) {
      const chosen = currentAvailableTasks[selectedTaskIndex];
      parts.push('我选择接取牝多多任务：' + chosen['任务名']);
      currentAvailableTasks = null;
      selectedTaskIndex = -1;
    }
  ...
  pDoc.addEventListener('click', function (e) {
    const secaskOpt = e.target.closest('#momo-status-bar .msb-secask-option');
    if (secaskOpt) {
      e.preventDefault();
      e.stopPropagation();
      const mainInput = pDoc.querySelector('textarea#send_textarea');
      if (!mainInput) return;
      const nameEl = secaskOpt.querySelector('.msb-task-option-name');
      if (!nameEl) return;
      const optText = nameEl.textContent.trim();
      mainInput.value = (mainInput.value.trim() ? mainInput.value.trim() + '\n' : '') + optText;
      mainInput.focus();
      return;
    }
    ...
    const closetPageBtn = e.target.closest('.msb-closet-page-btn');
    if (closetPageBtn) {
      const dir = closetPageBtn.dataset.page;
      const bar = closetPageBtn.closest('#momo-status-bar');
      const panel = bar?.querySelector('.msb-panel.closet');
      if (!panel) return;
      const cd = getData();
      const total = Math.max(1, Math.ceil(Object.keys(cd?.['衣柜']?.['可选服装'] || {}).filter(n => {
        const cur = cd?.['衣柜']?.['当前穿着'];
        return Array.isArray(cur) ? !cur.includes(n) : !momoWearList.includes(n);
      }).length / 6));
      if (dir === 'next') { closetPage = (closetPage + 1) % total; }
      else { closetPage = (closetPage - 1 + total) % total; }
      panel.innerHTML = renderClosetPanel(cd);
      return;
    }
  ...
  eventOn(tavern_events.MESSAGE_RECEIVED, message_id => {
    try {
      ...
      // Refresh contacts panel only (rebuild contacts list)
      pDoc.querySelectorAll('#momo-status-bar .msb-panel.contacts').forEach(panel => {
        panel.innerHTML = renderContactsPanel(getData());
      });
    } else {
...
/* ============================
   可选回复模块已迁移至状态栏底部 renderSecaskPanel
   ============================ */
