...
const metroSegments = computed(() => {
  ...
  const E = (): number[] => [];
  const busCentroidX = busRoutes.map(route => {
    let sx = 0, n = 0;
  ...
  for (let ri = 0; ri < metroRoutes.length; ri++) {
    ...
    for (let i = 0; i < route.length - 1; i++) {
      ...
      if (seen.has(key)) {
        ...
        for (let s = info.start; s < info.start + info.count; s++) {
        ...
        if (hasBus && hasRoad) {
          ...
          segs.push({ x1: x1+3*dx, y1: y1+3*dy, x2: x1+4*dx, y2: y1+4*dy, color: '#d44', busRoutes: [...busIdx], metroRoutes: [ri], triggerRoute: firstGetsSmall ? (busIdx[1] ?? busIdx[0]) : busIdx[0] });
          ...
          segs.push({ x1: x1+3*dx, y1: y1+3*dy, x2, y2, color: '#d44', busRoutes: [...busIdx], metroRoutes: [ri], triggerRoute: firstGetsSmall ? (busIdx[1] ?? busIdx[0]) : busIdx[0] });
          ...
          seen.set(key, { start: base, count: 1 });
        }
      }
    }
  }
  ...
  for (let ri = 0; ri < busRoutes.length; ri++) {
    ...
    for (let i = 0; i < route.length - 1; i++) {
      ...
      if (roadEdgeSet.value.has(key)) {
        ...
        segs.push({ x1: x1+2*dx, y1: y1+2*dy, x2: x1+3*dx, y2: y1+3*dy, color: '#d44', busRoutes: [...er], metroRoutes: [], triggerRoute: firstGetsSmall ? (er[1] ?? er[0]) : er[0] });
        segs.push({ x1: x1+3*dx, y1: y1+3*dy, x2, y2, color: '#444', busRoutes: [...er], metroRoutes: [], triggerRoute: -1 });
      } else {
        seen.set(key, segs.length);
        segs.push({ x1, y1, x2, y2, color: '#d44', busRoutes: [...er], metroRoutes: [], triggerRoute: er[0] });
      }
    }
  }
  return segs;
...
.map-bus-lines line.bus-hover, .map-metro-lines line.bus-hover { stroke: #d44 !important; stroke-width: 6; }
...
.schedule-date { font-size: 0.75rem; font-weight: bold; color: var(--c-accent); padding: 2px 0; border-bottom: 1px solid var(--c-border); margin-bottom: 0px; }
.schedule-card { background: rgba(255,255,255,0.03); border: 1px solid var(--c-border); border-radius: 6px; padding: 6px 10px; cursor: pointer; transition: border-color 0.2s, background 0.2s; max-width: 200px; }
...
.schedule-card.expanded { border-color: var(--c-accent); background: rgba(255,255,255,0.05); max-width: none; }
.schedule-header { display: flex; justify-content: space-between; align-items: baseline; }
.schedule-name { font-size: 0.78rem; font-weight: bold; color: var(--c-text); }
.schedule-time { font-size: 0.65rem; color: var(--c-text-dim); white-space: nowrap; }
.schedule-loc { font-size: 0.65rem; color: var(--c-accent); margin-top: 1px; }
.schedule-body { margin-top: 6px; padding-top: 6px; border-top: 1px solid var(--c-border); }
.schedule-content { font-size: 0.7rem; color: var(--c-text-dim); line-height: 1.5; }
